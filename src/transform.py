...
def convert_list_to_dataframe(data_list: list | None, entity_name: str) -> pd.DataFrame | None:
    ...
    if data_list is None:
        logger.warning(f"No data (None) for entity conversion '{entity_name}'.")
        return None
    if not isinstance(data_list,list):
        logger.warning(f"Entity data '{entity_name}' are not a list(type: {type(data_list)}. Cannot convert.")
        return None
    if not data_list: # Empty list control
        logger.warning(f"Entity list '{entity_name}' is empy.")
        return None # Possible return empty DataFrame or None
    
    try:
        df = pd.DataFrame(data_list)
        logger.info(f"Data '{entity_name}' succesfully converted to DataFrame.")
        return df
    except Exception as e:
        logger.error(f"Error during conversion '{entity_name}' to DataFrame: {e}")
        return None
...
def transform_users(users_df: pd.DataFrame | None) -> pd.DataFrame | None:
    ...
    try:
        ...
        logger.error(f"Error during user data transformation: {e}")
        return None
...
def transform_products(products_df: pd.DataFrame | None) -> pd.DataFrame | None:
    ...
    try:
        ...
        cols_to_keep = [
            ...
            "rating", "stock", "brand", "reviews" 
        ]
        # Check if columns exists 
        existing_cols = [col for col in cols_to_keep if col in products_df.columns]
        products_df = products_df[existing_cols].copy() # <--- Použití .copy()

        # Renaming columns to snake_case
        products_df = products_df.rename(columns={"discountPercentage": "discount_percentage"}) # Upraveno na snake_case

        # Chenge data type
        products_df['price'] = pd.to_numeric(products_df['price'], errors='coerce')
        products_df['discount_percentage'] = pd.to_numeric(products_df['discount_percentage'], errors='coerce')
        ...
        if 'reviews' in products_df.columns:
            ...
            count_reviews = lambda review_list: len(review_list) if isinstance(review_list, list) else 0
            ...
            get_comments = lambda review_list: [
                review.get('comment', '') for review in review_list if isinstance(review, dict)
            ] if isinstance(review_list, list) else []
            ...
            products_df['review_comments'] = products_df['review_comments'].apply(lambda comments: ' | '.join(comments))
        ...
        logger.error(f"Error during products data transformation: {e}") 
        return None



def transform_carts(carts_df_raw: pd.DataFrame | None) -> tuple[pd.DataFrame | None, pd.DataFrame | None]:
    """Transform products DataFrame.
    ...
    try:
        ...
        logger.info(f"Cleaning data for 'carts' table...")
        # Columns pick for carts
        cols_for_carts = ["id",             # Cart ID
                        "userId",           # Users ID
        ...
        carts_cleaned = carts_cleaned.rename(columns={
            "id": "cart_id",
            "userId": "user_id",  
            "total": "cart_total",
        ...
        if "user_id" in carts_cleaned.columns:
            carts_cleaned["user_id"] = pd.to_numeric(carts_cleaned['user_id'],errors="coerce").astype('Int64')
        ...
        if "discounted_total" in carts_cleaned.columns:
            carts_cleaned['discounted_total'] = pd.to_numeric(carts_cleaned['discounted_total'])
        if "total_products" in carts_cleaned.columns:
            carts_cleaned['total_products'] = pd.to_numeric(carts_cleaned['total_products']).astype('Int64')
        if "total_quantity" in carts_cleaned.columns:
            carts_cleaned['total_quantity'] = pd.to_numeric(carts_cleaned['total_quantity']).astype('Int64')
        ...
        for _, cart_row in carts_df_raw.iterrows(): # iteration over original df
            ...
            if "products" in cart_row and isinstance(cart_row["products"], list):
                for product in cart_row["products"]: # iteration over list of products in selected cart
                    cart_items_list.append({
                        ...
                        "total": product.get("total"), # total price for product (quantity * price)
                        "discount_percentage": product.get("discountPercentage"),
                        "discounted_price": product.get("discountedPrice") # discoundet product price
                    })
            else:
                logger.warning(f"Cart Id {cart_id} is not a valid list of product.")
        
        # Create DataFrame for cart items
        if not cart_items_list:
            logger.warning("Items in carts was not found, 'cart_items' could not be created.")
            cart_items_cleaned = pd.DataFrame(columns=['cart_id',
                                                'product_id',
                                                'title',
                                                'quantity',
                                                'price',
            ...
            cart_items_cleaned = cart_items_cleaned.rename(columns={
                'discount_percentage': 'discount_percentage',
                'discounted_price': 'discounted_price'
            })
            # Datatype change
            cart_items_cleaned["product_id"] = pd.to_numeric(cart_items_cleaned["product_id"]).astype('Int64')
            cart_items_cleaned["quantity"] = pd.to_numeric(cart_items_cleaned["quantity"]).astype('Int64')
            ...
            cart_items_cleaned["discount_percentage"] = pd.to_numeric(cart_items_cleaned["discount_percentage"])
            cart_items_cleaned["discounted_price"] = pd.to_numeric(cart_items_cleaned["discounted_price"])
        
        # logger.info(f"Successful carts transformation. Created DataFrames: carts ({carts_cleaned.shape}), cart_items ({cart_items_cleaned.shape})")
        return carts_cleaned, cart_items_cleaned
        
    except Exception as e:
        logger.error(f"Error during carts data transformation: {e}")
...
if __name__ == "__main__":
    ...
    # Cesty k datům (relativně k transform.py)
    ...
    for name, path in paths.items():
        try:
            with open(path, "r", encoding='utf-8') as f:
                ...
                raw_data[name] = full_data.get(name,[]) # Getting list under the key, default-> empty list
                if not raw_data[name]:
                    logger.warning(f"I file {path} cant be find data under key '{name}'.")
        except FileNotFoundError as e:
            logger.error(f"File not found: {path}")
            raw_data[name] = None
        except json.JSONDecodeError as e:
            logger.error(f"Error during decoding json file {path}: {e}")
            raw_data[name] = None
        except Exception as e:
            logger.error(f"Unexpected file reading error {path}: {e}")
            raw_data[name] = None
